...
<%
    ...
    if (weekParam != null) {
        baseDate = LocalDate.parse(weekParam)
    ...
    List<Availability> lista =
            (List<Availability>)
                    request.getAttribute("schedules");

    List<Meeting> meetings =
            (List<Meeting>)
                    request.getAttribute("meetings");
    ...
    Map<Integer, List<Availability>> porDia =
            new LinkedHashMap<>();
...
<%
    Map<String, List<Meeting>> meetingsPorSlot =
            new HashMap<>();

    if (meetings != null && lista != null) {

        for (Meeting m : meetings) {

            for (Availability av : lista) {

                boolean mesmoDia =
                        m.getDayTime()
                                .getDayOfWeek()
                                .equals(av.getDayWeek());

                LocalTime inicioMeeting =
                        m.getDayTime()
                                .toLocalTime();

                LocalTime fimMeeting =
                        inicioMeeting.plusMinutes(
                                m.getDurationMinutes()
                        );

                boolean mesmoHorario =
                        !inicioMeeting.isBefore(av.getHourStart())
                                &&
                                !inicioMeeting.isAfter(av.getHourEnd().minusMinutes(1));
...
<head>
    ...
    <script>
        ...
        function openModal(
            ...
            document
                .getElementById('input-inicio')
                .value = inicio || '';

            document
                .getElementById('input-fim')
                .value = fim || '';

            document
                .getElementById('form-availability')
                .action =
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="text-center">
                ...
                <p class="text-sm text-slate-500">
                    <%= diasSemana[0].format(fullFmt) %>
                    até
                    <%= diasSemana[6].format(fullFmt) %>
                </p>
            ...
            ">
                ...
                ">
                    ...
                    <% for (Availability h : horariosDia) {
                        ...
                        List<Meeting> slotMeetings =
                                meetingsPorSlot.getOrDefault(
                                        slotKey,
                                        List.of()
                                );

                        boolean temMeeting =
                                !slotMeetings.isEmpty();
                    %>
                    ...
                    ">
                        ...
                        <div class="flex gap-2">
                            ...
                            <form
                                    action="${pageContext.request.contextPath}/autenticado/availability/remove"
